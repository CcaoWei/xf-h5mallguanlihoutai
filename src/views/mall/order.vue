  ...
  <div class="app-container">
      ...
      <el-table-column align="center" label="操作" width="200" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button v-permission="['POST /admin/order/ship']" v-if="scope.row.orderStatus==201" type="primary"
            size="mini" @click="handleShip(scope.row)">确认</el-button>
          <el-button v-permission="['POST /admin/order/upay']" v-if="scope.row.orderStatus==101" style="width:75px;"
            type="primary" size="mini" @click="handleUpay(scope.row)">确认付款</el-button>
          <el-button v-permission="['POST /admin/order/refund']" v-if="scope.row.orderStatus==202" type="primary"
            size="mini" @click="handleRefund(scope.row)">退款</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :visible.sync="orderDialogVisible" title="订单详情" width="800">
      ...
      <el-form :data="orderDetail" label-position="left">
        ...
        <el-form-item label="支付信息">
          <span>（支付渠道）{{orderDetail.order.payType=="zfb"?"支付宝支付":"微信支付"}}</span>
    ...
    <el-dialog :visible.sync="shipDialogVisible" title="发货">
      <el-form ref="shipForm" :model="shipForm" status-icon label-position="left" label-width="100px"
        ...
        <el-form-item label="快递公司" prop="selectedKd">
          <el-select style="width:300px;" v-model="selectedKd" placeholder="请选择快递公司">
            <el-option v-for="kd in kdData" :key="kd" :label="kd" :value="kd">
            </el-option>
          </el-select>
        </el-form-item>
        ...
        <!-- <el-form-item label="仓库" prop="selectedCk">
          <el-select style="width:300px;" v-model="selectedCk" placeholder="请选择仓库">
            <el-option v-for="ck in ckData" :key="ck.id" :label="ck.ckName" :value="ck.id">
            </el-option>
          </el-select>
        </el-form-item> -->
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getCkData() {
        allCkOrder({}).then(res => {
          if (res.data && res.data.data && res.data.data.ck) {
            this.ckData = [...res.data.data.ck]
          }
        })
      },
      confirmShip() {
        ...
        //   return;
        // }
        console.log(this.selectedKd);
        console.log(this.shipForm.shipSn);
        ...
        if (this.shipForm.shipSn == "" || this.shipForm.shipSn == undefined || this.shipForm.shipSn == 'undefined') {
          this.$notify.error({
            title: '提交失败',
            message: '请填写快递编号'
          })
        ...
        this.$refs['shipForm'].validate((valid) => {
          if (valid) {
            this.shipForm.shipChannel = this.selectedKd;
            shipOrder(this.shipForm).then(response => {
              // shipOrder(Object.assign({}, this.shipForm, {
              // shipChannel: '',
              // shipSn: '',
              // selectedCk: '',
              // })).then(response => {
